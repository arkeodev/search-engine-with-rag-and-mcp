...
async def create_rag(links: List[str]) -> FAISS:
    ...
    try:
        ...
        logger.info(f"Created {len(split_documents)} document chunks from {len(documents)} documents")
...
async def create_rag_from_documents(documents: List[Document]) -> FAISS:
    """
    Create a RAG system directly from a list of documents to avoid repeated web fetching.
